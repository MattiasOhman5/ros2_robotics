...
def publish_nodes(nodes_xy, pub, frame="map"):
    m = Marker()
...
def publish_edges(edges_xy, pub, frame="map"):
    m = Marker()
    m.header.frame_id = frame
    m.header.stamp = rclpy.clock.Clock().now().to_msg()
    m.ns = "rrt"
    ...
    m.action = Marker.ADD
    m.scale.x = 0.03  # linjebredd
    m.color.r, m.color.g, m.color.b, m.color.a = 0.2, 0.4, 1.0, 1.0

    for (x1, y1), (x2, y2) in edges_xy:
        p1 = Point(); p1.x, p1.y, p1.z = float(x1), float(y1), 0.0
        p2 = Point(); p2.x, p2.y, p2.z = float(x2), float(y2), 0.0
...
class PathPlannerNode(Node):
    ...
    def plan_path(self,
        ...
        for goal in goals:
            ...
            if i % 20 == 0:
                nodes_xy = planner.node_coords
                edges_xy = edges_from_parent(planner.nodes)
                publish_nodes(nodes_xy, self.node_pub, frame=self.global_frame)
                publish_edges(edges_xy, self.edge_pub, frame=self.global_frame)
            ...
            total_cost = self.total_cost(path_cost, info_gain, path_weight=3.0, info_weight=5.0)
