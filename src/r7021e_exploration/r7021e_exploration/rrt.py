...
class RRT():
  def __init__(self, node, occupancy_grid, num_iterations, goal):
    # Lägger till ett dictionary för att catcha collision trädet då det blir många färre checkar
    self.collisionCache = {}
    ...
    self.tree_size = 1
    self.step_size = 0.25 # 0.3 -> 0.25
    self.sample_goal_bias = 0.05
    self.goal = goal
    self.r_goal = 0.3 # 0.15 -> 0.3
  ...
  def sample_free(self):
    ...
    x = self.og.info.origin.position.x + \
        random.uniform(0, self.og.info.width * self.og.info.resolution)
    y = self.og.info.origin.position.y + \
        random.uniform(0, self.og.info.height * self.og.info.resolution)
    x_rand = (x, y)
    return x_rand
  ...
  def informed_sample(self, x_start, x_goal, c_best):
    ...
    # formeln för denna är skriven i artikeln, Algorithm 2: rad 6
    r2 = math.sqrt(self.best_cost**2 - c_min**2) / 2.0

    # Algorithm 2: rad 8, SampleUnitBall
    u = random.random()
    v = random.random()
    # Man tar roten ur här för uniform distribution, annars klumpar sig punkterna runt origo
    r = math.sqrt(u)
    angle = 2.0 * math.pi * v
  ...
  def collision_free(self, x_nearest, x_new):
    key = (round(x_nearest[0], 2), round(x_nearest[1], 2),
           round(x_new[0], 2), round(x_new[1], 2))

    if key in self.collisionCache:
      return self.collisionCache[key]

    result = self._checkCollision(x_nearest, x_new)
    self.collisionCache[key] = result
    return result
    return True

  def _checkCollision(self, x_nearest, x_new):
  ...
  def world_to_grid(self, point):
    x, y = point
    gx = int((x - self.og.info.origin.position.x) /
             self.og.info.resolution)
    gy = int((y - self.og.info.origin.position.y) /
             self.og.info.resolution)
    return gx, gy
  ...
  def run_RRT(self):
    ...
    while i < self.num_iterations:
      ...
      x_rand = self.informed_sample(
          (0.0, 0.0), (self.goal[0], self.goal[1]), self.best_cost)
      ...
      if self.collision_free((x_nearest.x, x_nearest.y), x_new):
        ...
        x_new_node.cost = x_nearest.cost + \
            self.line_cost(x_new, (x_nearest.x, x_nearest.y))
        ...
        for node in x_near:
          if self.collision_free((node.x, node.y), x_new):
            cost = node.cost + \
                self.line_cost((node.x, node.y), x_new)
            if cost < best_cost:
              best_parent = node
              best_cost = cost
      ...
      # som dess parent
        for node in x_near:
          ...
          potentialCost = x_new_node.cost + \
              self.line_cost((x_new_node.x, x_new_node.y), (node.x, node.y)
                             )  # Disance check för att kippa den kostliga comperasen
          if potentialCost >= node.cost:
            continue

          if self.collision_free((x_new_node.x, x_new_node.y), (node.x, node.y)):
            cost = x_new_node.cost + \
                self.line_cost(
                    (x_new_node.x, x_new_node.y), (node.x, node.y))
            if cost < node.cost:
              if node.parent:
      ...
      # vi sparar den bäst väg vi hittar eftersom vi kör den en bestämd mängd iterationer
        if self.in_goal_region(x_new):
          if x_new_node.cost < self.best_cost:
            ...
            print(
                f"Found new best node, cost = {self.best_cost:.3f}")
...
def publish_rrt_markers(node: Node, rrt: RRT, node_pub, edge_pub, frame="map"):
  ...
  for n in rrt.nodes:
    if n.parent is not None:
      p1 = Point()
      p1.x, p1.y, p1.z = n.parent.x, n.parent.y, 0.0
      p2 = Point()
      p2.x, p2.y, p2.z = n.x, n.y, 0.0
