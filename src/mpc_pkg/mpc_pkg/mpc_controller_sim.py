...
class MPC_Controller(Node):
    ...
    def defineTBotMPC(self, model, ts, N):
        ...
        for x_obs, y_obs, r_obs, obs_name in obstacles:
            ...
            # --- CAP---
            # Cap center in global frame
            cap_x = model.x['x'] + self.cap_offset * ca.cos(model.x['th'])
            cap_y = model.x['y'] + self.cap_offset * ca.sin(model.x['th'])

            # Vector from cap center to obstacle
            dx_global = x_obs - cap_x
            dy_global = y_obs - cap_y

            # Rotate to cap's local frame for rectangle check
            dx_local = dx_global * ca.cos(model.x['th']) + dy_global * ca.sin(model.x['th'])
            dy_local = -dx_global * ca.sin(model.x['th']) + dy_global * ca.cos(model.x['th'])

            # Distance to rectangle edges
            dx_to_edge = ca.fmax(0, ca.fabs(dx_local) - self.cap_half_length)
            dy_to_edge = ca.fmax(0, ca.fabs(dy_local) - self.cap_half_width)
            dist_to_cap = ca.sqrt(dx_to_edge**2 + dy_to_edge**2)
